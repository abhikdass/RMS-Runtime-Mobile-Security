...
function spoofAdvertisingId(advertising_id) {
    if (advertising_id == RANDOM) {
        advertising_id = _randomHex(16);
    } else if (advertising_id !== null) {
        advertising_id = String(advertising_id).toLowerCase();
        if (! advertising_id.match(/^[0-9a-f]{16}$/)) {
            throw new Error("Invalid Advertising ID value");
        }
    }
    ...
    AdvertisingIdClient.getAdvertisingIdInfo.overload("android.content.Context").implementation = function(context) {
        var result = Java.use("com.google.android.gms.ads.identifier.AdvertisingIdClient$Info").$new(advertising_id, true);
        return result;
    }
}
