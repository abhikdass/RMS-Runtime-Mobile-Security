...
function setHttpProxy() {
    try {
        ...
        if (System) {
            ...
            send("HTTP proxy set: " + proxyIp + ":" + proxyPort);
        }
        ...
        if (Proxy && InetSocketAddress && String) {
            ...
            send("Required classes not found.");
        }
    } catch (error) {
        send("Error in setHttpProxy: " + error);
    }
}
...
function hookApplication() {
    try {
        ...
        Application.onCreate.implementation = function () {
            this.onCreate();
            send("Hooking application...");
            setHttpProxy();
        };
    } catch (e) {
        send("Error hooking application: " + e);
    }
}
